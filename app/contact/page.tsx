    ...
    <>
      ...
      <Container maxWidth="xl" sx={{ py: { xs: 7, md: 10 } }}>
        <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', lg: '0.72fr 1.28fr' }, gap: 3 }}>
          <Stack spacing={2.2}>
            <Reveal>
              <Card><CardContent sx={{ p: 3 }}><Stack direction="row" spacing={1.4}><Box sx={{ width: 46, height: 46, borderRadius: 2.3, bgcolor: 'primary.main', color: '#fff', display: 'grid', placeItems: 'center' }}><GroupsRoundedIcon /></Box><Box><Typography variant="h5" fontWeight={800}>PNC Student Team</Typography><Typography color="text.secondary" sx={{ mt: 0.6 }}>Seven members across development, QA, planning, data and telecom.</Typography></Box></Stack></CardContent></Card>
            </Reveal>
            <Reveal delay={0.05}>
            ...
            <Reveal delay={0.1}>
              <Card><CardContent sx={{ p: 3 }}><Typography variant="h6" fontWeight={800}>Contact a specific member</Typography><Stack spacing={1.1} sx={{ mt: 2 }}>{teamMembers.filter((member) => member.email).map((member) => <Box key={member.slug} component="a" href={`mailto:${member.email}`} sx={{ display: 'flex', gap: 1.2, alignItems: 'center', p: 1.1, borderRadius: 2, border: '1px solid', borderColor: 'divider', '&:hover': { borderColor: 'primary.main' } }}><Avatar src={member.photo} alt={member.name} sx={{ width: 40, height: 40 }} /><Box sx={{ minWidth: 0 }}><Typography fontWeight={800}>{member.name}</Typography><Typography variant="caption" color="text.secondary" sx={{ wordBreak: 'break-all' }}>{member.email}</Typography></Box></Box>)}</Stack></CardContent></Card>
            </Reveal>
          </Stack>
