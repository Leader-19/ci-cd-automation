      ...
      <Card sx={{ height: '100%', display: 'flex', flexDirection: 'column', overflow: 'hidden', transition: 'box-shadow 180ms ease', '&:hover': { boxShadow: '0 6px 18px rgba(16,24,40,0.08)' } }}>
        ...
        <CardContent sx={{ display: 'flex', flex: 1, flexDirection: 'column', p: { xs: 2, md: 2.5 } }}>
          ...
          <Stack direction="row" spacing={1} sx={{ mt: 'auto', pt: 2.2 }}>
            <Button component={Link} href={`/team/${member.slug}`} variant="contained" size="small" endIcon={<ArrowForwardRoundedIcon />} sx={{ flex: 1, minWidth: 0 }}>View Profile</Button>
            <Tooltip title={member.cv ? `Open ${member.name} CV` : 'CV not available'}>
              <span>
                {member.cv ? (
                  ...
                  <Button disabled variant="outlined" size="small" aria-label={`${member.name} CV is not available`} sx={{ width: 44, minWidth: 44, px: 0 }}><DescriptionOutlinedIcon fontSize="small" /></Button>
                )}
              </span>
            </Tooltip>
          </Stack>
