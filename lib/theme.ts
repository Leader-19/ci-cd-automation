...
export const buildTheme = (mode: 'light' | 'dark', locale: Locale = 'en') => {
  ...
  return createTheme({
    ...
    components: {
      ...
      MuiButton: { styleOverrides: { root: { minHeight: 40, borderRadius: 8, padding: '0 16px', boxShadow: 'none', transition: 'transform 160ms ease, box-shadow 160ms ease, background-color 160ms ease, border-color 160ms ease', '&:hover': { boxShadow: 'none', transform: 'translateY(-1px)' }, '&:active': { transform: 'translateY(0)' } }, sizeSmall: { minHeight: 36, fontSize: '12.5px', padding: '0 12px' }, sizeLarge: { minHeight: 44, padding: '0 18px' } } },
      ...
      MuiOutlinedInput: { styleOverrides: { root: { minHeight: 42, borderRadius: 8 }, input: { fontSize: '14px', padding: '10px 14px' } } },
