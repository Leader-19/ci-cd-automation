...
export default async function MemberPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const member = memberBySlug(slug);
  if (!member) notFound();
  const khmerCard = khmerTeamCardCopy[member.slug];
    ...
    <>
            ...
            <Reveal delay={0.05}>
              <Box>
                ...
                <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1.2} flexWrap="wrap" sx={{ mt: 3 }}>
                  {member.cv && <Button component="a" href={member.cv} download variant="contained" startIcon={<DownloadRoundedIcon />}><LocalizedText en="Download CV" km="ទាញយក CV" /></Button>}
                  {member.cv && <Button component="a" href={member.cv} target="_blank" rel="noreferrer" variant="outlined" startIcon={<OpenInNewRoundedIcon />}><LocalizedText en="Open CV" km="បើក CV" /></Button>}
      ...
      <Container maxWidth="xl" sx={{ py: { xs: 7, md: 10 } }}>
        ...
        <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', lg: 'minmax(0, 0.72fr) minmax(0, 1.28fr)' }, gap: 3.5 }}>
          <Stack spacing={3.5}>
            <Reveal>
              <Card><CardContent sx={{ p: 3 }}><Typography variant="h5" fontWeight={800}><LocalizedText en="About" km="អំពី" /></Typography><Typography color="text.secondary" sx={{ mt: 1.4, lineHeight: 1.85 }}><LocalizedText en={member.summary} km={khmerProfileSummaries[member.slug] || member.summary} /></Typography></CardContent></Card>
            </Reveal>

            <Reveal>
              <Card><CardContent sx={{ p: 3 }}><Typography variant="h5" fontWeight={800}><LocalizedText en="Contact" km="ទំនាក់ទំនង" /></Typography><Stack spacing={1.6} sx={{ mt: 2 }}>
