...
export const buildTheme = (mode: 'light' | 'dark') => {
  ...
  return createTheme({
    ...
    components: {
      MuiCssBaseline: { styleOverrides: { html: { scrollBehavior: 'smooth', WebkitTextSizeAdjust: '100%' }, body: { margin: 0, fontSize: '14px', overflowX: 'hidden' }, 'button, input, textarea, select': { fontFamily: 'inherit' }, '::selection': { backgroundColor: isLight ? '#155EEF' : '#4C8DFF', color: '#FFFFFF' }, a: { color: 'inherit', textDecoration: 'none' }, ':focus-visible': { outline: `2px solid ${isLight ? '#155EEF' : '#4C8DFF'}`, outlineOffset: 2 } } },
